  ...
  return (
    <div className="bg-gray-100 mt-8">
      <div className="py-10 md:px-32 px-5 rounded-lg shadow-lg bg-white">
        ...
        <ul className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4 h-56  ">
          {project.content[0].testimonials.map(
            (testimonial: string, index: number) => (
              <>
                <li
                  key={index}
                  className="shadow-md bg-white p-4 rounded-md mt-1  border  relative h-[11rem]"
                >
                  {testimonial}
                  <div className="flex absolute bottom-5  items-center gap-4">
                    <p className="border-2 border-[#41B98C] rounded-full p-2 ">
                      {/* <IoPersonOutline
                        size={20} */}
                      {/* className="border-2 border-[#41B98C] rounded-full p-2"
                      /> */}
                    </p>
